...
int main() {
    ...
    double L = pow((double)N / rho, 1.0/3.0);
    ...
    for(int idt=0; idt<ndt; idt++){
        ...
        sprintf(filename, "thermodynamics_VERLET_dt_%.4f.dat", dt);
        ...
        for(int step=0; step<steps; step++){
            if(step % 10 == 0){
                ...
                fprintf(f, "%d %.6f %.6f %.6f %.6f %.6f %.6f %.6f\n",step, step*dt, KE, PE, TE, PX, PY, PZ);
            }

            //euler_step(x, y, z, vx, vy, vz, fx, fy, fz, L, dt);
            verlet_velocity_step(x, y, z, vx, vy, vz, fx, fy, fz, L, dt);
        }
        fclose(f);
